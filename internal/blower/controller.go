...
func (c *Controller) Run(ctx context.Context, cmd model.BlowerCommand) (model.BlowerResult, error) {
	...
	for {
		...
		case <-ctx.Done():
			...
			c.phase = model.BlowerIdle
			c.mu.Unlock()
			return model.BlowerResult{OperationID: cmd.OperationID, Phase: model.BlowerIdle, TargetPct: target, ActualPct: c.ActualPct(), Cancelled: true, Message: ctx.Err().Error(), CompletedAt: time.Now().UTC()}, ctx.Err()
		case <-ticker.C:
			c.mu.Lock()
			if c.tripped {
				c.mu.Unlock()
